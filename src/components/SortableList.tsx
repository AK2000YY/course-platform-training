...
export function SortableList<T extends { id: string }>({
  ...
  const [optimisticItem, setOptimisticItem] = useOptimistic(items);
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    startTransition(async () => {
      setOptimisticItem(getNewArray(items, activeId, overId));
      const actionData = await onOrderChange(
        getNewArray(optimisticItem, activeId, overId).map((s) => s.id)
      );
      if (actionData.error)
        toast.error("Error", { description: actionData.message });
      else toast.success("Success", { description: actionData.message });
    });
  }
  ...
  return (
    <DndContext id={dndContextId} onDragEnd={handleDragEnd}>
      ...
      >
        <div className="flex flex-col">{children(optimisticItem)}</div>
      </SortableContext>
    </DndContext>
  );
}
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isActive && "z-10 shadow-md border"
      )}
    >
      <GripVerticalIcon
